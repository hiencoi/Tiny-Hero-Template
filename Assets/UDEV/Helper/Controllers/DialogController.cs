...
namespace UDEV
{
    ...
    public class DialogController : Singleton<DialogController>
    {
        ...
        private void OnOneDialogClosed(Dialog dialog)
        {
            ...
            if (current == dialog)
            {
                var hidingClip = Helper.GetClip(current.anim, current.hidingAnimation.clipName);

                if (current.anim && hidingClip)
                    waitingTime = hidingClip.length;

                current = null;
                dialogs.Pop();
                if (onDialogsClosed != null && dialogs.Count == 0)
                {
                    Timer.Schedule(this, waitingTime + 0.1f, () =>
                    {
                        onDialogsClosed();
                    });
                }

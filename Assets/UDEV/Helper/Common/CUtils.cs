...
namespace UDEV
{
	...
	private static AndroidJavaObject obj_Activity;
        ...
        {
            if (JobWorker.Ins.onLink2Store != null)
            {
                JobWorker.Ins.onLink2Store();
            }
            OpenStore();
            SetRateGame();
        }
        ...
        public static void OpenStore()
        {
            ...
            Application.OpenURL("https://play.google.com/store/apps/details?id=" + ConfigController.Ins.config.androidPackageID);
#elif ANDROID_NATIVE && UNITY_ANDROID
		obj_Activity.Call("linkGooglePlay");
#elif UNITY_IPHONE
		Application.OpenURL("https://itunes.apple.com/app/id" + ConfigController.Ins.config.iosAppID);
		...
		Application.OpenURL("fb://page/" + ConfigController.Ins.config.facebookPageID);
#else
            Application.OpenURL("https://www.facebook.com/" + ConfigController.Ins.config.facebookPageID);
#endif
        ...
        public static void LoadScene(int sceneIndex, bool useScreenFader = false)
        {
            if (useScreenFader)
            {
                ScreenFader.Ins.GotoScene(sceneIndex);
            }
            else
            {
...
#else
                if (JobWorker.Ins.onShowInterstitial != null)
                {
                    JobWorker.Ins.onShowInterstitial();
...
#if UNITY_ANDROID || UNITY_IPHONE
        AdmobController.Ins.RequestBanner();
#endif
        }
...
#if UNITY_ANDROID || UNITY_IPHONE
        AdmobController.Ins.ShowBanner();
#else
            if (JobWorker.Ins.onShowBanner != null) JobWorker.Ins.onShowBanner();
#endif
        }
        ...
        {
#if UNITY_ANDROID || UNITY_IPHONE
        AdmobController.Ins.HideBanner();
#else
            if (JobWorker.Ins.onCloseBanner != null) JobWorker.Ins.onCloseBanner();
#endif
        }
        ...
        public static void ShowFixedBannerAd()
        {
            if (IsBuyItem()) return;
            if (IsAdsRemoved()) return;
            if (JobWorker.Ins.onShowFixedBanner != null) JobWorker.Ins.onShowFixedBanner();
        }
